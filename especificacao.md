...
| **Fluxo Alternativo:**  |  ```sh <strong>Fluxo Alternativa A:</strong> <br> a: O ator decide efetuar cadastro na plataforma <br> b: Insere os dados requisitados <br> c: O sistema verifica algum erro nos dados inseridos <br> d: O sistema encontra o erro <br> e: Retorna a letra b do Fluxo Principal para refazer o processo. ```  <br> <strong>Fluxo Alternativa B: </strong> <br> a: O ator conclui com sucesso o cadastro e entra no sistema. <br> b: Procura o botão de acessar seus dados <br> c: Muda os dados de sua preferência <br> d: Confirma as modificações <br> e: Sistema valida os dados e confirma modificações. |
